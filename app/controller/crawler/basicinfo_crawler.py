...
class BasicinfoCrawler:
	...
	def save_user(self,
		...
		try:
			name = user.name.replace('\\','\\\\').replace("'","\\'").replace(':','\\:')
			...
			sql = """INSERT INTO %s (user_id, screen_name, name, location, created_at, description, statuses_count, friends_count,
					...
					'%s', '%s', '%s', '%s', '%s')""" % (table_name, user.id, user.screen_name, name, location, user.created_at, description, \
					user.statuses_count, user.friends_count, user.followers_count, user.favourites_count, user.lang, protected, time_zone, verified, \
					utc_offset, geo_enabled, user.listed_count, default_profile_image, user.profile_background_color, \
					user.profile_sidebar_fill_color, user.profile_image_url, profile_banner_url, time.strftime('%Y-%m-%d',time.localtime(time.time()))) 
