...
def GenerateUserXml(twitter_user):
    ...
    InfluenceT = dom.createTextNode(str(twitter_user['influence_score']))
    InfluenceE.appendChild(InfluenceT)

    # 用户社交影响力标签
    RankInfluE = dom.createElement("影响力等级")
    RankInfluT = dom.createTextNode(str(twitter_user['rank_influ']))
    ...
    return config.XML_path + '%s.xml' % twitter_user['screen_name']
